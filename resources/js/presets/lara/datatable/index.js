export default {
  ...
  wrapper: ({ props }) => ({
    class: [
      ...
      // Size
      { 'h-full': props.scrollable && props.scrollHeight === 'flex' },
      'relative overflow-hidden border-gray-200 shadow sm:rounded-lg',
    ],
  }),
  header: ({ props }) => ({
    class: [
  ...
  column: {
    headercell: ({ context, props }) => ({
      class: [
        'whitespace-nowrap text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer',
        ...
        // Spacing
        context?.size === 'small' ? 'py-3 px-6' : context?.size === 'large' ? 'p-5' : 'p-4',
        // 'py-3 px-6',
        // Color
    ...
    bodycell: ({ props, context, state, parent }) => ({
      class: [
        ...
        // Alignment
        'text-left divide-y divide-red-200',
  ...
  bodyrow: ({ context, props }) => ({
    class: [
      ...
      { 'odd:bg-surface-0 odd:text-surface-600 dark:odd:bg-gray-800 even:bg-gray-50 even:text-gray-600 dark:even:bg-gray-900': context.stripedRows },
